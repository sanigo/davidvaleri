...
public class ExampleCallbackHandler implements CallbackHandler {
    ...
    /**
     * Returns a live copy of the alias to password {@code Map} for decryption
     * keys. The returned {@code Map} is suitable for runtime configuration.
     * 
     * @return a live copy of the alias to password {@code Map} for decryption
     *         keys
     */
    public Map<String, String> getDecryptCredentials() {
     ...
     *            signature keys
     */
    public void setDecryptionCredentials(Map<String, String> decryptCredentials)
    {
        this.decryptCredentials.clear();

        if (decryptCredentials != null) {
            this.decryptCredentials.putAll(decryptCredentials);
        }
    }
        ...
        for (Callback callback : callbacks) {
            ...
            switch (pc.getUsage()) {
                ...
                case WSPasswordCallback.SIGNATURE:
                    ...
                    break;
                // Case for decryption key password.
                case WSPasswordCallback.DECRYPT:
                	this.handleDecrypt(pc);
                	break;
                default:
                    throw new UnsupportedCallbackException(callback, 
     ...
     * more details.  The caller expects the implementation to set
     ...
     * Handle a request for the password to access the private key for the
     * provided alias ({@link WSPasswordCallback#getIdentifier()}). Refer to
     * {@link WSPasswordCallback} and {@link WSPasswordCallback#DECRYPT} for
     * more details.  The caller expects the implementation to set
     * the password field of the callback 
     * ({@link WSPasswordCallback#setPassword(String)).
     * <p/>
     * This implementation returns the matching password from the
     * {@link #decryptCredentials signature credentials map}.
     * 
     * @throws IOException
     *             if there is an error processing the request
     * @throws UnsupportedCallbackException
     *             if the alias does not have credentials in the map
     */
    protected void handleDecrypt(WSPasswordCallback pc) throws IOException,
            UnsupportedCallbackException
    {
        final String password = this.decryptCredentials.get(pc
                .getIdentifier());
